...
<Note>
  Prefer not to hand-roll HTTP calls? The [official libraries](/integrations/libraries) wrap this API for Node.js (`filecheck-node`) and PHP (`filecheck/filecheck-php`) — typed clients with retries, uploads, webhooks, and a one-call `verify()` — plus browser packages for the Element (`filecheck-js`, `filecheck-react`, `filecheck-vue`).
</Note>
