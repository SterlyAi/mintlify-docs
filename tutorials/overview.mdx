...
<CardGroup cols={2}>
  <Card title="Element on a custom stack" icon="code" href="/tutorials/api-integration">
    No plugin? Embed the Element yourself and verify the job server-side.
  </Card>
  <Card title="Headless API integration" icon="server" href="/integrations/headless">
    No widget at all? Submit files with `POST /jobs` and collect results by webhook or polling.
  </Card>
</CardGroup>
