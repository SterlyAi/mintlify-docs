...
5. Assign an **Action** for each check to decide what happens when a file misses the target. Every check uses one of the same [four actions](/configuration/preflight#the-four-actions):
   - **Reject**: Blocks the file at intake. The customer must fix and re-upload.
   - **Warn**: Flags the issue but lets the customer proceed.
   - **Auto-fix**: Filecheck corrects the issue automatically in the workflow's processing phase.
