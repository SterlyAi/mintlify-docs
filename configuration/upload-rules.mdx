...
<Frame caption="Where a Rule sits: the intake gate inside a workflow that's assigned to a product">
  <iframe src="https://cdn.filecheck.io/videos/rule.html" title="How an Upload Rule works" loading="lazy" style={{ width: "100%", aspectRatio: "16 / 9", border: "0", borderRadius: "12px" }} />
</Frame>
...
<Note>
  ...
  {/* TODO(filecheck): list the exact vector and office formats accepted for conversion (e.g. AI/EPS/SVG, DOCX/PPTX/…). */}
  **TODO(filecheck): list the exact vector and office formats accepted for auto-conversion at intake.**
</Note>
...
<Note>
  **Two layers decide the outcome.** Each check inside a Preflight Profile has its own [action](/configuration/preflight#the-four-actions) (Reject / Warn / Auto-fix / Off), and the Rule adds this overall **onFail** policy on top. When they disagree — for example a check set to **Reject** under a Rule set to **Accept with warnings** — the resolved outcome is:

  {/* TODO(filecheck): confirm and document precedence. Does a check-level Reject always
      block regardless of the Rule's onFail, or can onFail override it? State the rule of thumb. */}

  **TODO(filecheck): document which layer wins when a check action and the Rule's onFail policy disagree.**
</Note>
